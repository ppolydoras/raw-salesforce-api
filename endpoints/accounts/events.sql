...
-- @param event_subject the subject of the event related with a given account. Substring search is supported.
-- @type event_subject varchar
-- @default event_subject null
-- @param event_id the ID of the event related with a given account.
...
-- @return Products related to opportunities based on filters like account name, account id, and product name.

WITH filtered_account_events AS (
    ...
    FROM salesforce.salesforce_event e
             INNER JOIN salesforce.salesforce_account a ON a.id = e.what_id
